...
public class GraphicsManager : MonoBehaviour
{
    ...
    private void InitializeResolutionDropdown()
    {
        ...
        //loop to check which resolution should be selected and shown and refresh UI 
        for (int i = 0; i < resolutionDropdown.dropdownItems.Count; i++)
        {
            Resolution res = resolutions[i];
            if (res.width == savedResolution.width && res.height == savedResolution.height && res.refreshRateRatio.value == savedResolution.refreshRateRatio.value)
            {
    ...
    //courutine so that all dropdowns,buttons etc. initiliaze first in other script then change selected item in this script
